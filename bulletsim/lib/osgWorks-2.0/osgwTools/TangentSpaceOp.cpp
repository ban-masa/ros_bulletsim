...
TangentSpaceOp::operator()( osg::Geometry& geom )
{
    ...
    if( geom.getVertexAttribArray( _normalIndex ) == NULL )
        geom.setVertexAttribArray( _normalIndex,
            _tsg->getNormalArray(), osg::Array::BIND_PER_VERTEX);

    if( geom.getVertexAttribArray( _tangentIndex ) == NULL )
        geom.setVertexAttribArray( _tangentIndex,
            _tsg->getTangentArray(), osg::Array::BIND_PER_VERTEX);

    if( geom.getVertexAttribArray( _binormalIndex ) == NULL )
        geom.setVertexAttribArray( _binormalIndex,
            _tsg->getBinormalArray(), osg::Array::BIND_PER_VERTEX);
