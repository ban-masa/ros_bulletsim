...
ColorCloudPtr smoothSurface(const ColorCloudPtr in) {
  ...
  std::cout << "smoothSurface should not be used." << std::endl;
  for (pcl::PointCloud<PointNormal>::iterator it = mls_points.begin(); it != mls_points.end(); it++) {
    ColorPoint p;
    p.x = it->x;
    p.y = it->y;
    p.z = it->z;
...
void labelCloud(ColorCloudPtr in, const cv::Mat& labels) {
  ...
  for (int i=0; i < in->size(); i++)
    ...
    //_unused is no longer the member slot of XYZRGB
    //Therefore, I replaced it with a, but I'm not sure it is correct.
