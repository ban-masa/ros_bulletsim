...
static BulletObject::Ptr createFromLink(KinBody::LinkPtr link,
	...
	const std::vector<KinBody::Link::GeometryPtr> &geometries =
			link->GetGeometries();
	...
	for (std::vector<KinBody::Link::GeometryPtr>::const_iterator it =
			geometries.begin(); it != geometries.end(); ++it) {
    const KinBody::Link::GeometryPtr& geom = (*it);
