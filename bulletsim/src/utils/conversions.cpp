...
btTransform toBulletTransform(const tf::Transform& transform)
{
  double matrix1[15];
  float matrix2[15];
  transform.getOpenGLMatrix(matrix1);
  for (int i = 0; i < 15; i++) matrix2[i] = (float)matrix1[i];
  btTransform t;
  t.setFromOpenGLMatrix(matrix2);
  return t;
}
...
Eigen::Affine3f toEigenTransform(const tf::Transform& transform) {
  ...
  Eigen::Affine3f t2 = t.cast<float>();
  return t2;
}
...
tf::Transform toTfTransform(const btTransform& transform)
{
  float matrix1[15];
  double matrix2[15];
  transform.getOpenGLMatrix(matrix1);
  for (int i = 0; i < 15; i++) matrix2[i] = (double)matrix1[i];
  tf::Transform t;
  t.setFromOpenGLMatrix(matrix2);
  return t;
}
